...
export default class BinanceDataFeed {
  ...
  subscribe(symbols, callbacks) {
    ...
    if (unavailableStreamKeys.length) {
      ...
      if (!isBrowser()) subscription.on("pong", this.handlePong);
    } else {
  ...
  unsubscribe(subscription) {
    let id = subscription;
    if (typeof subscription === "object") id = subscription.id;
  ...
  heartbeat(subscriptions) {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (!sub.isActive) {
        sub.onError(
          "WebSocket connection is no longer active. Binance did not return a response it time."
        );
        sub.close(sub);
      }
