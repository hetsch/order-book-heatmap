...
export default class BinanceOrderBook {
  ...
  constructor(feed, symbol, tickSize) {
    ...
    setTimeout(() => {
      feed.getOrderBookSnapshot(symbol, this.initFromSnapshot.bind(self));
    }, 2000);
  }
  ...
  initFromSnapshot(snapshot) {
    ...
    this.bookBuffer.forEach((update) => {
      if (update.U > snapshot.lastUpdateId) this.onDepth(update);
    });
  }
  ...
  onTrade(data) {
    if (!this.isSnapshotParsed) return;
    ...
    if (trade.isBuy) {
      ...
      this.stats.mktBuySize = this.tick.round(
        this.stats.mktBuySize + trade.size
      );
      this.stats.mktBuyPriceTotal += trade.price * trade.size;
    } else {
      this.stats.mktSellOrders++;
      this.stats.mktSellSize = this.tick.round(
        this.stats.mktSellSize + trade.size
      );
  ...
  getSnapshot(levels = 25, aggregation = 10) {
    if (levels <= 0 || aggregation <= 0) throw "Invalid aggregation step";

    if (!this.isSnapshotParsed) return null;
  ...
  aggregateBids(levels) {
    ...
    for (let i = 0; i < levels; i++) {
      ...
      for (let j = max; j >= min; j = this.tick.decr(j)) {
        if (this.book[j]) size += this.book[j];
      }
