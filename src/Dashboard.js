...
export default class Dashboard {
  ...
  updateDashboard(snapshot) {
    ...
    this.y = this.y.concat(snapshot.aggAskPrices.slice(0, this.levels - 1));
    ...
    const maxOrderbookLength =
      maxSeriesLength * (this.levels + this.bufferLevels) * 2;
    if (this.orderbook.length > maxOrderbookLength) {
      this.orderbook = this.orderbook.slice(
        (this.levels + this.bufferLevels) * 2
      );
    }
    ...
    const totalTradedSize =
      snapshot.stats.mktBuySize + snapshot.stats.mktSellSize;
    let delta = {
      x: ts,
    ...
    const sortedTrades = this.trades
      ...
      .sort((a, b) => a - b);
    const top10PercentileIndex = Math.floor(
      sortedTrades.length - 1 - sortedTrades.length / 10
    );
  ...
  renderHeatmap() {
    ...
    const width =
      heatmapEl.clientWidth ||
      window.innerWidth * 0.66 - margin.left - margin.right;
    const height =
      window.innerHeight -
      margin.top -
      margin.bottom -
      uiBarEl.clientHeight -
      7;
    ...
    // x axis
    const x = d3.scaleBand().range([0, width]).domain(self.x);
    svg
      .append("g")
    ...
    ticks.each(function (_, i) {
      if (i % xLabelPeriod !== 0) d3.select(this).remove();
    });

    // y axis
    const y = d3.scaleBand().range([height, 0]).domain(self.y);
    svg
      .append("g")
    ...
    const getLogAskColor = d3
      .scaleLinear()
      .domain([0, Math.log2(this.maxDepth)])
      .range(askColorRange);

    const getLogBidColor = d3
      .scaleLinear()
      .domain([0, Math.log2(this.maxDepth)])
      .range(bidColorRange);
    ...
    const visibleBook = this.orderbook.filter(
      (d) => d.y >= this.y[0] && d.y <= this.y[this.y.length - 1]
    );

    // limit order heatmap segments
    svg
      .selectAll()
      .data(visibleBook, function (d) {
        return d.x + ":" + d.y;
      })
      .enter()
      .append("rect")
    ...
    const sDeltaValues = this.mktOrderDeltas
      .map((x) => x.value)
      .sort(numCompare);
    const maxTradedSize = Math.max(
      ...this.mktOrderDeltas.map((x) => x.totalSize)
    );
    ...
    svg
      ...
      .attr("r", (d) =>
        self.getDeltaDotRadius(d.totalSize, y.bandwidth(), maxTradedSize)
      )
      .style("fill", function (d) {
        if (d.type === "ask") {
  ...
  renderTimeAndSales() {
    ...
    for (let i = 0; i < this.trades.length; i++) {
      ...
      row.classList +=
        this.trades[i].size >= this.topTradeSize ? " top-trade" : "";
  ...
  renderLimitOrdersBarChart() {
    ...
    const xAxis = d3.scaleBand().range([0, width]).domain(this.y).padding(0.2);
    svg
      .append("g")
      .attr("transform", "translate(0," + height + ")")
      .attr("class", "axis-x")
    ...
    ticks.each(function (_, i) {
      if (i % xLabelPeriod !== nthLabel) d3.select(this).remove();
    });
    ...
    for (let l = this.orderbook.length - 1; l > 0; l--) {
      ...
      if (this.y.indexOf(lvl.y) === -1) continue;

      if (lvl.type === "ask") askLevels.push(lvl);

      if (lvl.type === "bid") bidLevels.push(lvl);
    }
  ...
  getDeltaDotRadius(size, vertBandwidth, maxTradedSize) {
    ...
    if (0) {
      baseMultiplier +=
        (Math.log2(size) / Math.log2(maxTradedSize)) * maxMultiplier;
    } else {
